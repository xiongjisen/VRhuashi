...
class FPicoMobilePlugin : public IPicoMobilePlugin
{
	...
	void RegisterSettings()
	{
		...
		if (SettingsModule != nullptr)
		{
			...
			UPicoMobileSettings* const Settings = GetMutableDefault<UPicoMobileSettings>();
	...
	virtual void StartupModule() override
	{
		...
		FCoreDelegates::OnFEngineLoopInitComplete.AddRaw(this, &FPicoMobilePlugin::RegisterSettings);
	}
...
bool FPicoMobileHMD::bIsFirstResume = true;
	...
	TrackingToWorld(FTransform::Identity)
{
	...
	UE_LOG(LogPicoHMD, Log, TEXT("SDK Version: %s; Plugin Version: 1.2.7.5"), *sdkVersion);
#endif
}
...
void FPicoMobileHMD::OnBeginRendering_RenderThread(FRHICommandListImmediate& RHICmdList, FSceneViewFamily& ViewFamily)
{
	...
	if (IsInitialized())
	{
		...
		static int DoOnece = 0;
		if (DoOnece == 0)
		{
			DoOnece = 1;
			theTime = FPlatformTime::Seconds();
		}
		if ((FPlatformTime::Seconds() - theTime) > 0.5f && DoOnece == 1)
		{
			DoOnece = 2;
			ExecuteOnRHIThread([this](){FPicoJava::HideLoading();});
		}
...
{
#if PLATFORM_ANDROID
	switch (PicoSettings->refreshRate)
	{
		case ERefreshRate::Default:
		{
			PicoSDK::Render::SetDisplayFrequency(-1.0f);
			break;
		}
		case ERefreshRate::RefreshRate72:
		{
			PicoSDK::Render::SetDisplayFrequency(72.0f);
			break;
		}
		case ERefreshRate::RefreshRate90:
		{
			PicoSDK::Render::SetDisplayFrequency(90.0f);
			break;
		}
		default:
			break;
...
bool FPicoMobileHMD::Startup()
{
	...
	PicoImport::PvrSetColorspaceType(IsMobileColorsRGB() ? 1 : 0);
...
#if PLATFORM_ANDROID	
	PicoSDK::Render::CurrentIPD = PicoImport::PvrGetIPD();
	if (IsInitialized())//&&!bIsFirstResume)
	{
		ExecuteOnRenderThread([this]()
		{
			ExecuteOnRHIThread([this]()
	...
	if (IsInitialized()&& LayerSortIndex.Num() > 0 )
	{
		...
		for (TPair<uint32, int32>& element : LayerSortIndex)
		{
			...
			if (LayerFound)
			{
				...
				FMatrix mMVMatrixLeft = leftOffsetMat * ((*LayerFound)->GetLayerMV(CamRotation, CamLocation, isFixOverlay, TrackingToWorld));
				FMatrix mMVMatrixRight = rightOffsetMat * ((*LayerFound)->GetLayerMV(CamRotation, CamLocation, isFixOverlay, TrackingToWorld));
