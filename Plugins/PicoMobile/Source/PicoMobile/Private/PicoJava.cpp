...
static inline void AndroidThunkCpp_PanelToolInit()
{
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		static jmethodID Method = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_UEPanelToolInit", "()V", false);
		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GameActivityThis, Method);
	}
}
...
void FPicoJava::PanelToolInit()
{
	AndroidThunkCpp_PanelToolInit();
}
