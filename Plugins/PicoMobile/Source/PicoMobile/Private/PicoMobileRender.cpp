	...
	FClearValueBinding::Black)
{
	...
	for (int i = 0; i < mkRenderTargetTextureNum; ++i)
	{
		...
		{
#if ENGINE_MINOR_VERSION > 25
			mRenderTargetTextureRHIRef[i] = RHICreateTexture2D(InSizeX, InSizeY, PF_R8G8B8A8, InNumMips, InNumSamples, InTargetableTextureFlags, CreateInfo);
#else	
...
{
#if ENGINE_MINOR_VERSION > 26 
	SetResource(GetRenderTargetResource());
#else
...
{
#if PLATFORM_ANDROID
	int MobileMSAAValue = 0;
	int msaa = 0;
	if (PicoSettings->bUseRecommendedMSAA)
	{
		PicoImport::PvrGetIntConfig(PicoSDKConfig::GlobalIntConfigs::AntiAliasingLevelRecommended, msaa);
		MobileMSAAValue = msaa;
	}
	else
	{
	...
	TargetableTextureFlags =IsMobileColorsRGB()? (TargetableTextureFlags | TexCreate_SRGB):TargetableTextureFlags;
