...
FPicoDisplayRefreshRateDelegate UPicoBlueprintFunctionLibrary::PicoDisplayRefreshRateCallback;
...
void UPicoBlueprintFunctionLibrary::PicoGetRefreshRate(EPicoRefreshRate& rate)
{
#if PLATFORM_ANDROID
	float srate = -1.0f;
	PicoSDK::Render::GetRefreshRate(srate);
	if (srate == -1.0f)
	{
		rate = EPicoRefreshRate::Default;
	}
	else if (srate == 72.0f)
	{
		rate = EPicoRefreshRate::RefreshRate72;;
	}
	else if (srate == 90.0f)
	{
		rate = EPicoRefreshRate::RefreshRate90;;
	}
	else
	{
		rate = EPicoRefreshRate::Default;;
...
void UPicoBlueprintFunctionLibrary::PicoSetRefreshRate(EPicoRefreshRate rate)
{
#if PLATFORM_ANDROID
	switch (rate)
	{
		case EPicoRefreshRate::Default:
		{
			PicoSDK::Render::SetRefreshRate(-1.0f);
			break;
		}
		case EPicoRefreshRate::RefreshRate72:
		{
			PicoSDK::Render::SetRefreshRate(72.0f);
			break;
		}
		case EPicoRefreshRate::RefreshRate90:
		{
			PicoSDK::Render::SetRefreshRate(90.0f);
			break;
		}
		default:
			break;
...
void UPicoBlueprintFunctionLibrary::PicoDisplayRefreshRateChangedDelegates(FPicoDisplayRefreshRateDelegate OnPicoDisplayRefreshRateChanged)
{
#if PLATFORM_ANDROID
	PicoDisplayRefreshRateCallback = OnPicoDisplayRefreshRateChanged;
#endif
}
...
extern "C" JNIEXPORT void  JNICALL Java_com_epicgames_ue4_GameActivity_nativePicoDisplayRefreshRateChanged(JNIEnv * env, jclass clazz, float rate)
{
	if (UPicoBlueprintFunctionLibrary::PicoDisplayRefreshRateCallback.IsBound())
	{
		UPicoBlueprintFunctionLibrary::PicoDisplayRefreshRateCallback.Execute(rate);
	}
}
