...
bool UPicoNeoControllerBPlib::GetMainControllerBatteryLevel(int32 &Power)
{
	Power = 0;
#if PLATFORM_ANDROID
	Power = FPicoNeoControllerJni::GetInstance()->GetMainControllerBatteryLevel();
#endif
	if (Power!=0)
	{
		return true;
...
bool UPicoNeoControllerBPlib::GetSubControllerBatteryLevel(int32 &Power)
{
	Power = 0;
#if PLATFORM_ANDROID
	Power = FPicoNeoControllerJni::GetInstance()->GetSubControllerBatteryLevel();
#endif
	if (Power!=0)
	{
		return true;
...
void UPicoNeoControllerBPlib::PicoSetControllerOriginOffset(int hand, FVector offset)
{
	if(hand == 0)
	{
		FPicoNeoController::originOffsetL = offset;
	}
	else if(hand ==1)
	{
		FPicoNeoController::originOffsetR = offset;
	}
}
